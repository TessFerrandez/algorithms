...
class Solution:
    ...
    def getAncestors2(self, n: int, edges: List[List[int]]) -> List[List[int]]:
        direct_ancestors = defaultdict(list)

        for fr, to in edges:
            direct_ancestors[to].append(fr)
        ...
        def get_all_ancestors(node):
            ...
            all_ancestors = direct_ancestors[node]
            for anc in direct_ancestors[node]:
                all_ancestors += get_all_ancestors(anc)
            direct_ancestors[node] = list(set(all_ancestors))
            return direct_ancestors[node]

        result = []
        for i in range(n):
            result.append(sorted(set(get_all_ancestors(i))))
        return result
    ...
    def getAncestors(self, n: int, edges: List[List[int]]) -> List[List[int]]:
        ...
        all_ancestors = [set() for _ in range(n)]

        for fr, to in edges:
            ancestors[to].append(fr)
        ...
        while supplies:
            ...
            for child in children[node]:
                all_ancestors[child].add(node)
                all_ancestors[child] = all_ancestors[child].union(all_ancestors[node])
                ancestors[child].remove(node)
                if len(ancestors[child]) == 0:
                    supplies.add(child)

        return [sorted(nodes) for nodes in all_ancestors]
